...
def _get_range_and_validation_info(row: pd.Series, enums_df: pd.DataFrame) -> Dict[str, Any]:
...
def parse_table_df(df: pd.DataFrame, enums_df: Optional[pd.DataFrame] = None, detailed_enum_names: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    if "Submission Requirement" in df.columns:
        df["required"] = df["Submission Requirement"].map(lambda x: x.strip(" .").lower() in ["required"])
    else:
        df["required"] = False
    ...
    if detailed_enum_names is not None and len(detailed_enum_names) > 0:
        for enum_name in detailed_enum_names:
            ...
            df.loc[filt, "range"] = df.loc[filt].apply(lambda x: f"{enum_name}[{x['slot']}]", axis=1)

    return df

def extract_all_classes(metadata_file: Union[str, Path], enums_file: Union[str, Path], output_dir: Union[str, Path], single_table: Optional[bool] = False, detailed_enum_names: Optional[List[str]] = None):
    ...
    for table_name, table_df in all_tables.items():
        ...
        cur_df = parse_table_df(table_df, enums_df=enums_df, detailed_enum_names=detailed_enum_names)
...
if __name__ == "__main__":
    if "get_ipython" in globals():
        ...
        args.add_argument("--enums_file", type=str, help="Input enums (Value Sets) sheet from the NWSS data dictionary", required=False)
        args.add_argument("--output_dir", type=str, help="Directory to save the Schemasheets classes files to", required=True)
        args.add_argument("--single_table", action="store_true", help="If set then merge all classes into a single class, otherwise keep them as separate classes")
    ...
    extract_all_classes(opts.metadata_file, opts.enums_file, opts.output_dir, single_table=opts.single_table, detailed_enum_names=opts.detailed_enum_names)
